...
Note that computing Nth order derivatives using reverse-mode autodiff is more computationally expensive than computing first order derivatives. (Most deep learning frameworks only compute first order derivatives. Widely used optimisation methods, such as [Adam](https://arxiv.org/abs/1412.6980), somewhat approximate higher order derivatives from the first order derivatives.)
