...
- `local_gradients` - the node's children & corresponding 'local derivatives'.
...
- Multiply the edges of a path (into a node).
- Add together the different paths (that lead to a node).

The tuples in `stack` (in `get_gradients`) are similar 
to the tuples in `grad`, but they contain the 
current path value, instead of the local derivative value.
